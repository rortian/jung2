...
public class SimpleSparseGraphTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        graph.addDirectedEdge(e21, v2, v1);
    ...
    public void testGetInOutEdges() {
        ...
        assertEquals(graph.getOutEdges(v1).size(), 3);
        assertEquals(graph.getOutEdges(v0).size(), 2);
    }
    ...
    public void testIsDirected() {
        for(Number edge : graph.getEdges()) {
        	if(edge == e21) {
        		assertTrue(graph.isDirected(edge));
        	} else {
        		assertFalse(graph.isDirected(edge));
        	}
        }
    }
}
